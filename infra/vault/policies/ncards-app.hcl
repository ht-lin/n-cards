...
# 加了就要在 PR 里写明这个问题的答案。
#
# ⚠️⚠️ **改了这个文件，它不会自己到长期运行的环境上去。**
# `bootstrap.sh` 是幂等覆盖的，但它在 staging / 生产上**只跑过首启那一次**。
# 真正把这份文件送进 Vault 的是每次部署跑的 `infra/vault/policy-sync.sh`
# （ansible 的 vault-policy 任务）。T-114 之前没有那个执行者，后果是
# 2026-09-19：T-104 在 9-07 加的 JWT 那条路径始终没到 staging，
# `POST /auth/otp/verify` 503 而所有绿灯都是绿的。
# 改完之后合入即生效，但**部署完要确认那一步真的绿了** —— 它是唯一的判据。
#
# ============================================================================
# 为什么没有 sys/health
# ============================================================================
