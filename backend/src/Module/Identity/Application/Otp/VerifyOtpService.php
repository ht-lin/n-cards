 ...
 * ============================================================================
 * **拒绝路径**（401）是安全敏感的：它要对「挑战不存在」「码错了」「过期了」
 * 「已经用过了」「试太多次了」五种情形返回**逐字相同**的响应，
 * 且做功与耗时也要相同（见下一节）。
 *
 * （这里原本还有第六种「这是上个版本留下的哑挑战」。T-113 摘掉了那次判断，
 * 理由见 {@see verify()} 里布尔链上的注释。）
 *
 ...
 *   2. **`hash_equals` 永远执行**，即使 `$challenge` 是 null、即使前面几项已经
 *      注定这次要拒。它是拒绝路径上唯一一次 Vault 往返，也就是唯一一项
 *      可测量的开销 —— 把它放进任何分支里，那个分支就变得可测量地更快。
 *      所以 {@see verify()} 里 `$codeHash` 的**位置是安全约束，不是风格**。
 *
 *      ⚠️ 这一条原本还有后半句：「`isDecoy()` 永远排在 `hash_equals` 后面」。
 *      T-113 摘掉了那次判断（ADR-0014 之后哑挑战早已不存在，见下），
 *      于是布尔链里不再有任何一项与「这个邮箱注册过吗」相关。前半句仍然成立。
...
final readonly class VerifyOtpService
{
     ...
     *
     * @throws DomainException                             401 `token_invalid`（挑战不存在/码错/过期/已用/次数耗尽）、
    ...
    public function verify(OtpVerificationPayload $payload, ?string $clientIp): SessionIssued
    {
        ...
        // ⚠️ 次序是安全约束（见类注释第 2 条）。`&&` 会短路，但这四项都是纯内存判断，
        // 唯一有代价的一步已经在上面执行完了 —— 短路在这里不产生可测量的差异。
        //
        // ============================================================
        // T-113：这里原本还有第五项 `&& !$challenge->isDecoy()`
        // ============================================================
        // 它是 ADR-0014 之前「哑挑战」机制的读取端：邮箱不存在时建一条不发信、
        // 验证恒失败的挑战。ADR-0014（T-104）把两条路径合并成恒发码之后，
        // 那一列**再无写入方**，留着这次判断只是为了让部署窗口内库里那些
        // 上个版本建的哑挑战继续 401 —— 而它们的寿命是 10 分钟。
        //
        // 摘掉它是安全的，有三重理由，任意一条单独成立即可：
        //   1. ADR-0014 上线于 2026-09-06，哑挑战早已一条不剩（10 分钟寿命）。
        //   2. 哑挑战的 `code_hash` 对应一个**从未发出过**的码；要走到这条分支，
        //      攻击者得在 §7.1 给的 5 次尝试内猜中 6 位数字。
        //   3. 本卡的 PurgeDeadOtpChallengesTask 首跑会把任何残留行删干净。
        //
        // ⚠️ 列、映射与 `OtpChallenge::isDecoy()` **都还留着**，不是遗漏：
        // `doctrine:schema:validate`（CI 的 migration:check 第 ③ 步）比的是映射与
        // 真库，映射先于列消失会让它报「不同步」。两者必须同一次发布一起删，
        // 那是 §13.5 的 N+2，归后续卡（见 docs/tasks/M1.md 的 T-113 交接清单）。
