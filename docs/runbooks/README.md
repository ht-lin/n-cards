...
| [`email-dns.md`](email-dns.md) | 发信域的 SPF / DKIM / DMARC（`p=none`→`quarantine`→`reject` 分三阶段推进）与**通道切换**（R1 的处置：改 `MAILER_DSN` + 重启 worker）。⚠️ Q3 未决，DKIM selector 仍是占位符 | T-102 | ✅ 初版（DNS 记录待 Q3）|
