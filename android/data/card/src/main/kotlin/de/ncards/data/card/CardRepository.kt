...
interface CardRepository {
    ...
    /**
     * 一张卡，**在当前用户视角下**（T-154 的卡详情页与全屏条码页）。
     *
     * ============================================================================
     * ⚠️ `null` 有两种含义，调用方必须分开处理
     * ============================================================================
     * - **这张卡不在我的钱包里**：它从来就不是我的，或者一次下行同步刚把它删掉
     *   （owner 删卡 / 我被移出成员）。这是**正常结局**，不是错误 ——
     *   详情页正开着时完全可能发生，界面该说「这张卡已不在你的钱包里」。
     * - **还不知道我是谁**：冷启动时令牌要过一趟 Keystore 才读得出来。
     *
     * 两者靠 [observeCurrentUserId] 区分，与 [observeWallet] 是同一条约定。
     *
     * ============================================================================
     * ⚠️ 它建在 [observeWallet] 之上，**不是**一句新 SQL
     * ============================================================================
     * 详情页要 `role`（gate 编辑入口）、`isPinned`、`sortOrder` 与派生的
     * `sync_state`，也就是 `CardDao` 里那段带两个 JOIN 和一个派生子查询的查询 ——
     * 而那段 SQL 在 `CardDao` 里**已经逐字存在两份**（`observeWallet` 与
     * `walletSnapshot`），它自己的 KDoc 写着「改上面那段的人必须同时改这一段」。
     * 第三份的漂移代价高于一次内存过滤：§7.5 的每用户上限是 500 张卡，
     * 规格自注「P99 < 30」。
     *
     * 这与 T-153 把搜索留在内存里用的是同一条理由。
     */
