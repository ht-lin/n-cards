 ...
 * 今天两条路径的实际 ABI 都是 x86_64，62 次测试（31 × 两档）在 api 26 与 api 34 上全绿，
 * 本地与 CI 都验证过（CI 首次跑通见 T-011 的第四层修复，run 33526439179）。
 * ⚠️ 历史教训留着：这 62 个绿一度**只在本地**跑过，当时被拿来当作「CI 上也能跑」的
 * 依据，而 CI 那时一台模拟器都起不来 —— 无头 runner 与本地的差异（GPU、磁盘余量）
 * 是实打实的。**别再拿本地绿去论证 CI**，判据见 android/README.md 那张四层表。
