 ...
 *
 * 这里只有**应用外壳**的配色。卡片调色板（用户给每张卡选的颜色）在
 * `CardColorScheme.kt`，键在 `core:model` 的 `CardColor`。
 *
 * ⚠️ **本文件此前写着「不要在这里先编几个色值顶上」——那条禁令由 T-153 解除，
 * 但解除它的不是「我编得准」。** 原话的担忧是「§11.2 要求每个卡片色都逐个校验
 * ≥ 4.5:1，编出来的值必然过不了那一关，而届时已经有 UI 代码引用它们了」。
 * 化解它的是 `CardColorContrastTest`：那一关现在**自己会跑**，
 * 换色值当场就知道过不过得了。Q7 因此是「工程侧闭环，待设计复核」——
 * 复核时改的是 `CardColorScheme.kt` 的色值，`CardColor` 的键与已落库的数据都不动。
 *
 * §11.2 的对比度要求对下面这些同样适用。⚠️ 但它们**没有**那样一条自动化断言 ——
 * 外壳配色是 Material 3 的 `ColorScheme`，前后景的配对由 Material 自己决定
 * （而且 Android 12+ 上还会被动态取色整个换掉），没法像卡片色那样逐对校验。
 * 改动下面任何一个色值，仍然要人工过一遍对比度。
 */
