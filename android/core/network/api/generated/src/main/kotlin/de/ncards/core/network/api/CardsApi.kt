...
interface CardsApi {
     ...
     * POST cards
     * 创建一张卡
     * ⚠️ &#x60;id&#x60; 由**客户端生成**（UUIDv7）。这是离线优先的前提：用户在超市地下层 没网时加的卡，本地就已经有了最终 id，联网后原样上行——不需要「本地临时 id → 服务端真 id」的重映射（§5.4.3）。  因此本端点**天然幂等**：同一个 &#x60;id&#x60; 重复提交返回同一张卡。若该 &#x60;id&#x60; 已属于 **别人**，返回 &#x60;409 id_conflict&#x60;，客户端必须重新生成 id 重试。  限额（§7.5）：每用户 500 张卡，超出 &#x60;422 limit_exceeded&#x60;。 共享给我的卡**不计入**我的额度（Q11）。 
     * Responses:
     *  - 201: 已创建
