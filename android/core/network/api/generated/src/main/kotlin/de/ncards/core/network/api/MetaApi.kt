...
interface MetaApi {
     ...
     * GET config
     * 客户端运行时配置
     * 强制升级基线、维护窗口公告与 feature flag（§3.10、§6.2、§9.2）。 **无需 Bearer**，客户端在**每次启动时**拉一次。  ⚠️ **一个过旧的客户端在这里拿到的是 &#x60;426&#x60;，不是配置。** &#x60;X-Client&#x60; 的强制 升级判定对 &#x60;/v1/_*&#x60; 一视同仁，本端点不例外。这不是缺陷：&#x60;426&#x60; 本身就是 强制升级墙的信号（T-158 的拉取逻辑正是「调本端点 → &#x60;426&#x60; → 弹墙」）， 客户端不需要先读到 &#x60;latest_client&#x60; 才知道该弹墙。&#x60;latest_client&#x60; 的消费者 是**仍在支持范围内**的客户端——它用来做「有新版可用」的软提示。  ⚠️ &#x60;maintenance.message_key&#x60; 是**本地化键，不是文案**。面向用户的德语文案 由客户端生成（§11.1）；窗口时间本身固定为每周二 03:00–04:00 CET（§9.2）， 所以文案里可以写死它，本端点不下发时间戳。  ⚠️ &#x60;feature_flags&#x60; 里**永远不会出现 §7.5 的限额数字**。那些是服务端强制的 绝对上限，客户端持有一份可能过期的副本只会制造「本地预校验说没满、服务端 说满了」这种无法由客户端判断对错的矛盾（T-111 移交笔记）。  本端点是整个 &#x60;/v1&#x60; 里**唯一可缓存**的响应（&#x60;Cache-Control: public, max-age&#x3D;60&#x60;， §7.4 的 &#x60;no-store&#x60; 在此例外：它不含任何个人数据）。响应带 &#x60;Vary: X-Client&#x60; —— 响应**体**与该头无关，但**状态码**与它强相关（过旧 → 426），少了 &#x60;Vary&#x60; 的共享缓存会把 200 喂给旧客户端，升级墙就再也不出现了。  **限流**：按 IP **300/min**（§7.5）。超限是 &#x60;429&#x60;，按 &#x60;Retry-After&#x60; 退避。 配额按分钟而非小时、且留足 CGNAT 余量，因为这是客户端每次冷启动都会拉的端点。  ⚠️ 本端点的限流是 §7.5 里**两条 fail-open 策略**之一 （[ADR-0021](https://github.com/ht-lin/n-cards/blob/main/docs/adr/0021-second-fail-open-rate-limit-for-the-config-endpoint.md)）： 限流器自身不可用时本端点**仍然返回 200**，不会 503。所以客户端的退避逻辑 只按 &#x60;429&#x60; 写 —— 这里的 &#x60;503&#x60; 永远不是「限流器无法判定」，而是真的服务不可用。 
     * Responses:
     *  - 200: 当前配置
