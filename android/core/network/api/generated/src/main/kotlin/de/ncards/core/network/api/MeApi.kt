...
import de.ncards.core.network.api.model.MeUpdate
...
interface MeApi {
     ...
     * GET me
     * 我的资料
     * **&#x60;onboarding_incomplete&#x60; 状态下仍然可达的三个端点之一**（另外两个是 &#x60;POST /me/username&#x60; 与 &#x60;POST /auth/logout&#x60;，§5.2）。客户端正是靠这里的 &#x60;user.onboarding_complete&#x60; 决定下一步去哪：&#x60;false&#x60; → username 设定页 （不可跳过、不可返回）；&#x60;true&#x60; → 钱包。  所以它**必须**免受 onboarding 拦截 —— 把它挡住，客户端就再也问不出 「我是不是卡在注册中间态」，注册流程无法完成。 
     * Responses:
     *  - 200: 我的资料。示例特意用注册中间态（`username: null` / `onboarding_complete: false`）—— 那是这个端点最要紧的一种响应。 
     ...
     */
    @GET("me")
    suspend fun getMe(@Header("X-Client") xClient: kotlin.String): Response<UserEnvelope>
     ...
     * PATCH me
     * 修改我的资料
     * 目前只能改 &#x60;locale&#x60;。  ⚠️ **它不在 onboarding 白名单里。** 未设 username 的用户调它得到 &#x60;403 username_required&#x60; —— §5.2 / §6.2 的豁免清单里只有 &#x60;GET /me&#x60;。 在 username 设定之前开一条写 &#x60;users&#x60; 的路径，正好是那个拦截器要挡的事； 而注册期的语言由 &#x60;POST /auth/otp/request&#x60; 的 &#x60;locale&#x60; 字段决定， 不需要这个端点。  ⚠️ **请求体里出现 &#x60;username&#x60; → &#x60;409 username_immutable&#x60;，不静默忽略** （§6.2）。这是那条「username 不可变」不变量在本端点上的落点： 静默忽略会让客户端以为改成功了，而下一次 &#x60;GET /me&#x60; 才发现没有 —— 那种不一致比一个明确的 409 难查得多。  没有 &#x60;display_name&#x60;（v1.1 C10 移除），将来也不会有。  ⚠️ **空对象 &#x60;{}&#x60; 返回 &#x60;400 malformed_request&#x60;，不是 200 no-op。** &#x60;locale&#x60; 是当前唯一的字段，因此 &#x60;required&#x60;；一个什么都不改的 PATCH 只可能是客户端 bug。将来加了通知偏好之后，这条会放宽成「至少给一个字段」 —— 那是 §13.6 允许的放宽方向（收紧才是禁止的）。 
     * Responses:
     *  - 200: 改完之后的完整资料。**与 `GET /me` 同一个 `UserEnvelope`** —— 客户端拿它直接替换本地副本，不需要再拉一次。 
     *  - 400: `validation_failed`（字段校验失败，带 `errors[]`）或 `malformed_request`（body 不是合法 JSON / 不是 JSON 对象 / 为空）。  缺失或格式错误的 `X-Client`、缺失的 `If-Match`、以及任何 offset 风格的 分页参数（`offset` / `page` / `skip` / `per_page` / `start`， `errors[].code = unsupported_parameter`）也都走这里。 
     *  - 401: `token_expired`（静默刷新后重试**一次**）或 `token_invalid`（会话已撤销 → 清空本地会话，跳登录，**不要重试**）。 
     *  - 403: `insufficient_role`（viewer 试图改卡/删卡/邀请——**同时上报 Sentry**， 正常 UI 不应产生此请求）、`not_a_member`（从本地删除该卡）、 `username_required`（跳 username 设定页）、`not_friends`。  ⚠️ `username_required` 不是某几个端点的特例，而是**整个 `/v1` 面**的规则： `username IS NULL` 的用户除 `GET /me`、`POST /me/username`、 `POST /auth/logout` 外，调任何操作都得到它（§5.2，由单一 Kernel 监听器 强制）。所以每个操作的 `403` 都可能是它 —— 客户端的处置固定为 「跳 username 设定页」，不需要按端点分支。 
     *  - 409: `revision_conflict`（乐观锁失败，`current` 带服务端状态 → 走 §5.4.3 冲突解决）、 `id_conflict`（客户端生成的 id 已属于他人 → **重新生成 id 重试**）、 `already_exists`（幂等处理）、`idempotency_in_progress`（带 `Retry-After`，退避重试）、 `full_resync_required`（清库全量重同步）、 `username_taken` / `username_immutable`（见 `/me/username` 与 `PATCH /me`）。 
     ...
     */
    @PATCH("me")
    suspend fun updateMe(@Header("X-Client") xClient: kotlin.String, @Body meUpdate: MeUpdate): Response<UserEnvelope>
